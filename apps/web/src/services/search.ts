...
export const searchService = {
  ...
  getSuggestions: (keyword: string) =>
    request.get<SearchSuggestion[]>('/search/suggestions', { keyword }),

  // 获取热门关键词
  getHotKeywords: (limit: number = 10) =>
    request.get<HotKeyword[]>('/search/hot-keywords', { limit }),

  // 获取搜索历史
  getSearchHistory: (limit: number = 20) =>
    request.get<SearchHistory[]>('/search/history', { limit }),
  ...
  getRecommendations: (userId?: number, limit: number = 10) =>
    request.get<KnowledgeItem[]>('/recommendations', { userId, limit }),

  // 获取个性化推荐
  getPersonalizedRecommendations: (limit: number = 10) =>
    request.get<KnowledgeItem[]>('/recommendations/personalized', { limit }),
  ...
  getTrendingContent: (
    ...
    request.get<KnowledgeItem[]>('/recommendations/trending', { timeRange, limit }),

  // 获取相似内容推荐
  getSimilarContent: (knowledgeId: number, limit: number = 5) =>
    request.get<KnowledgeItem[]>(`/recommendations/similar/${knowledgeId}`, { limit }),
  ...
  submitRecommendationFeedback: (
    ...
    request.post('/recommendations/feedback', { knowledgeId, feedback }),
};
